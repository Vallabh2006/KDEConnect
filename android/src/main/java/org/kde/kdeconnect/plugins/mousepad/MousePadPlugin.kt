...
class MousePadPlugin : Plugin() {
    ...
    override fun getUiButtons(): List<PluginUiButton> = emptyList()
    ...
    fun sendBackspace() {
        val np = NetworkPacket(PACKET_TYPE_MOUSEPAD_REQUEST)
        np["specialKey"] = KeyListenerView.SpecialKeysMap.get(KeyEvent.KEYCODE_DEL, 1)
        sendPacket(np)
    }

    fun sendReturn() {
        val np = NetworkPacket(PACKET_TYPE_MOUSEPAD_REQUEST)
        np["specialKey"] = KeyListenerView.SpecialKeysMap.get(KeyEvent.KEYCODE_ENTER, 12)
        sendPacket(np)
    }

    fun sendSpecialKey(keyCode: Int) {
        val np = NetworkPacket(PACKET_TYPE_MOUSEPAD_REQUEST)
        np["specialKey"] = KeyListenerView.SpecialKeysMap.get(keyCode, -1)
        sendPacket(np)
    }
