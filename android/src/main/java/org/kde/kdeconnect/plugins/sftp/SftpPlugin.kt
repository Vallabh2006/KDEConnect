...
class SftpPlugin : Plugin(), OnSharedPreferenceChangeListener {
    ...
    override fun onPacketReceived(np: NetworkPacket): Boolean {
        if (np.type == PACKET_TYPE_SFTP) {
            val ip = np.getString("ip")
            val port = np.getInt("port", -1)
            val user = np.getString("user", "kdeconnect")
            val password = np.getString("password")
            val path = np.getString("path", "/")
            val error = np.getStringOrNull("errorMessage")
            ...
            if (ip.isNotEmpty() && port > 0) {
                val sftpUrl = "sftp://$user:$password@$ip:$port$path"
                Handler(Looper.getMainLooper()).post {
                    ClipboardListener.instance(context).setText(sftpUrl)
                    val intent = Intent(Intent.ACTION_VIEW, Uri.parse(sftpUrl)).apply {
                        addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                    }
                    try {
                        context.startActivity(intent)
                    } catch (_: Exception) {
                        Toast.makeText(context, "PC SFTP ready at $ip:$port (URL copied)", Toast.LENGTH_LONG).show()
                    }
                }
                return true
            }
    ...
    override val supportedPacketTypes: Array<String> = arrayOf(PACKET_TYPE_SFTP_REQUEST, PACKET_TYPE_SFTP)

    override val outgoingPacketTypes: Array<String> = arrayOf(PACKET_TYPE_SFTP, PACKET_TYPE_SFTP_REQUEST)
