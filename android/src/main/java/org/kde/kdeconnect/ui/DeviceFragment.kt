                ...
                KdeTheme(context) {
                    ...
                    when (device.pairStatus) {
                        PairingHandler.PairState.Paired -> {
                            if (device.isReachable) {
                                val preferredButtonOrder = listOf(
                                    "Files",
                                    "Clipboard",
                                    "Stream",
                                    "Presentation",
                                ...
                                val pluginsWithButtons =
                                    device.loadedPlugins.values
                                        .flatMap { plugin -> plugin.getUiButtons() }
                                        .sortedWith(compareBy { button ->
                                            val idx = preferredButtonOrder.indexOf(button.name)
